...
export default function Paginas() {
  ...
  const loadPages = async () => {
    try {
      ...
      toast('Erro ao carregar páginas', { type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast('Página excluída!', { type: 'success' });
      loadPages();
    } catch (err) {
      toast('Erro ao excluir página', { type: 'error' });
    }
  };
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to="/admin/paginas/nova">
            <Button size="sm">Nova Página</Button>
          </Link>
        </div>
              ...
              <tbody>
                {pages.map((page: any) => (
                  <tr key={page.id} className="border-b border-gray-100 dark:border-gray-800">
                    <td className="py-3 px-4 text-gray-800 dark:text-white/90">{page.title}</td>
                    <td className="py-3 px-4 text-gray-600 dark:text-gray-400">{page.slug}</td>
