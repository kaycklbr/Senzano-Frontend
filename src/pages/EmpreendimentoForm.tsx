...
export default function EmpreendimentoForm() {
  ...
  const loadPost = async () => {
    try {
      ...
      toast('Erro ao carregar empreendimento', { type: 'error' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('type', 'empreendimento');
      ...
      if (isEdit) {
        ...
        toast('Empreendimento criado!', { type: 'success' });
      }
      
      navigate('/admin/empreendimentos');
    } catch (err) {
      toast('Erro ao salvar empreendimento', { type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <PageBreadcrumb 
        pageTitle={isEdit ? "Editar Empreendimento" : "Novo Empreendimento"} 
      />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="submit" loading={loading}>
              {isEdit ? 'Atualizar' : 'Criar'} Empreendimento
            </Button>
          </div>
        </form>
      </div>
